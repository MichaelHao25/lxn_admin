...
const SortComponent = () => {
  ...
  useEffect(() => {
    PageControllerFindOne({ type: IGlobalConfig.defaultProductImgConfig }).then(
      (res) => {
        if (res?.data?.indexShowType) {
          form.setFieldValue("defaultProductImage", [
            {
              uid: "1",
              name: res.data.defaultProductImage,
              status: "done",
              url: res.data.defaultProductImage,
            },
          ]);
        }
      }
